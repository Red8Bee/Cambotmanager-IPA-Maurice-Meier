...
from .camera import take_images
...
def _take_snapshot(snapshot, base_directory):
    ...
    depth_file = FileEntry('depth', depth_name, 'image/png', datetime.datetime.now())
    color_file = FileEntry('color', color_name, 'image/png', datetime.datetime.now())
